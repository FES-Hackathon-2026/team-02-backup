...
export default function Touren() {
  ...
  return <Screen back title={t(driver ? 'Meine Sammeltouren' : 'Fahrer:innen-Demo')} sub={t(driver ? 'Deine Stopps und Abholungen' : 'Beispieltour mit erfundenen Adressen')}>
    {t(!driver && <div className="card sky"><h1 className="h2">{t("Ein Fahrzeug, eine gemeinsame Tour")}</h1><p className="sm">{t("Die Referenzreise mit sieben fiktiven Stopps. Änderungen hier betreffen nur diese Vorschau. Bürger:innen sehen in ihren echten Anfragen ausschließlich den eigenen Stopp.")}</p><button className="btn sm" onClick={() => { setDemo(DEMO); setMessage('Demo zurückgesetzt.') }}>{t("Demo neu starten")}</button></div>)}
    {t(data.loading && <p role="status">{t("Touren laden …")}</p>)}
    {t(data.error && <button className="btn" onClick={data.reload}>{t("Touren erneut laden")}</button>)}
    ...
    {t(tours.map(tour => <section className="card" key={tour.id}>
      ...
      <svg viewBox="0 0 360 100" role="img" aria-label={t("Schematische Stoppreihenfolge, keine Straßenkarte")} style={{ width: '100%', background: 'var(--sky2)', borderRadius: 16 }}>
        ...
        {t(tour.stops.slice(0, 7).map((stop, i) => <g key={stop.id} transform={`translate(${25 + i * 50},${i % 2 ? 40 : 55})`}><circle r="13" fill="var(--blue-deep)" /><text textAnchor="middle" y="4" fill="var(--on-blue)" fontSize="12">{t(i + 1)}</text></g>))}
      </svg>
      <p className="xs mut">{t("Schematische Reihenfolge. Keine Live-Ortung oder verifizierte Straßenroute.")}</p>
      {t(driver && tour.status === 'collecting' && <button className="btn primary" disabled={busy} onClick={() => void action(`/api/fes/driver/tours/${tour.id}/plan`)}>{t("Demo-Tour planen und Fenster zuweisen")}</button>)}
      {t(tour.stops.filter(stop => filter === 'all' || stop.kind === filter).map(stop => <button key={stop.id} className="card tight" style={{ marginTop: 10 }} onClick={() => { setError(''); setSelected(stop) }}>
        <b>{stop.address}</b><p className="xs mut">{t(stop.eta || 'Zeitfenster folgt')} {t(" · ")}{t(stop.kind === 'fes' ? 'FES-Sammelpunkt (Demo)' : 'Gebucht')} {t(" · ")}{t(stop.status === 'collected' ? 'Abgeholt ✓' : 'Offen')}</p>
      </button>))}
    </section>))}
    ...
    {t(selected && <DecisionSheet title={t(`${selected.address} · ${selected.eta || 'noch ungeplant'}`)} busy={busy} onClose={() => setSelected(null)}>
      ...
      }}>{t(selected.status === 'collected' ? 'Bereits abgeholt ✓' : 'Abgeholt ✓')}</button>
