...
export default function Wissen() {
  ...
  return (
    <Screen
      back
      title="Was mache ich damit?"
      sub={eintrag ? eintrag.name : 'Abfall-ABC'}
      gap={13}
    >
      {/* the whole list stays reachable — an ABC is for browsing too */}
      <div className="chips scroll">
        {antwort.data?.entries.map((e) => (
          <button
            key={e.id}
            className="chip"
      ...
      {antwort.error && (
        <p className="sm" style={{ margin: 0 }}>
          ...
          <span className="mut">{antwort.error.message}</span>
        </p>
      )}

      {antwort.data && !eintrag && (
        <div className="empty">
          <Icon name="info" size={28} />
          {antwort.data.message ?? 'Oben auswählen, worum es geht.'}
        </div>
      )}
        ...
        <>
          ...
          <div className="card">
            <div className="row" style={{ gap: 12, alignItems: 'flex-start' }}>
              ...
              <span className="grow">
                <h1 className="h1" style={{ fontSize: 22 }}>{eintrag.bin}</h1>
                <span className="row" style={{ gap: 6, marginTop: 7, flexWrap: 'wrap' }}>
                  <Label>{ROUTE_LABEL[eintrag.route] ?? eintrag.route}</Label>
                  <Tag von="simulated" icon />
                </span>
              </span>
            </div>

            <p className="sm mut" style={{ margin: '13px 0 0', lineHeight: 1.6 }}>{eintrag.why}</p>
          </div>

          {/* 2 — when it is next emptied. A rule without a date is homework. */}
          {antwort.data?.nextDate && (
            <div className="card tight row" style={{ gap: 11 }}>
              ...
              <span className="grow">
                <b className="sm" style={{ display: 'block' }}>
                  Nächste Leerung: {antwort.data.nextDate.label}
                </b>
                <span className="xs mut">{antwort.data.nextDate.window}</span>
              </span>
          ...
          {antwort.data?.places && antwort.data.places.length > 0 && (
            <div className="card">
              <div className="between" style={{ marginBottom: 10 }}>
                <p className="lbl">{antwort.data.placeLabel}</p>
                <Tag von="api" icon>OpenStreetMap</Tag>
              </div>
              <div className="col" style={{ gap: 11 }}>
                {antwort.data.places.map((h) => (
                  <div key={h.id} className="row" style={{ gap: 11, alignItems: 'flex-start' }}>
                    <span style={{ marginTop: 2 }}>
                      <Icon name="pin" size={17} className="ico" />
                    </span>
                    <span className="grow">
                      <b className="sm" style={{ display: 'block' }}>{h.name}</b>
                      <span className="xs mut">
                        {h.addr ?? 'Frankfurt am Main'} · {entfernung(h.distanceKm)}
                        {h.openingHours ? ` · ${h.openingHours}` : ''}
                      </span>
                    </span>
                  </div>
                ))}
              ...
              <p className="xs mut" style={{ margin: '11px 0 0' }}>
                {antwort.data.attribution} · Luftlinie ab Stadtteilmitte
              </p>
            </div>
          )}
          ...
          <div className="card">
            <p className="lbl" style={{ marginBottom: 10 }}>Was nicht geht</p>
            <div className="col" style={{ gap: 9 }}>
              {eintrag.notAllowed.map((satz, i) => (
          ...
          {eintrag.reuse && (
            <div className="card sky">
              <div className="row" style={{ gap: 8, marginBottom: 7 }}>
                ...
                <b className="h3">{eintrag.reuse.titel}</b>
              </div>
              <p className="sm mut" style={{ margin: 0, lineHeight: 1.55 }}>{eintrag.reuse.text}</p>
              {eintrag.reuse.route && REUSE_ZIEL[eintrag.reuse.route] && (
                <button
                  className="btn sm"
                  style={{ marginTop: 11 }}
                ...
                >
                  ...
                  {REUSE_ZIEL[eintrag.reuse.route].label}
                </button>
              )}
            </div>
          )}
          ...
          <div className="card flat">
            <div className="between" style={{ marginBottom: 9 }}>
              <p className="lbl">Rechtsgrundlage</p>
              <Tag von="api">Gesetzestext</Tag>
            </div>
            <div className="col" style={{ gap: 8 }}>
              {eintrag.legal.map((satz, i) => (
                <div key={i} className="row" style={{ alignItems: 'flex-start', gap: 9 }}>
                  <span style={{ marginTop: 2 }}>
                    <Icon name="shield" size={15} className="ico" />
                  </span>
                  <span className="xs mut" style={{ lineHeight: 1.55 }}>{satz}</span>
                </div>
              ))}
            </div>
