...
export default function ScanTransport({ scan }: { scan: ScanResult }) {
  ...
  return <div className="card sky">
    ...
    {t(result?.candidates?.map(p => <button key={p.id} className="card tight" style={{ marginTop: 10 }} onClick={() => navigate(`/abholung?${new URLSearchParams({ photo: scan.photoId, address: p.address, district: p.districtId })}`, { state: { scan } })}>
