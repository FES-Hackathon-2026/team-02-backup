// Assembles every design/parts/<Name>.html body fragment into a Design
// Component artboard at design/<Name>.dc.html, injecting the shared stylesheet
// and the icon set. Run: node design/build.mjs
...
const css = readFileSync(join(parts, '_shared.css'), 'utf8')

// 24px grid, 1.7 stroke, round caps — one consistent family, no emoji.
const P = {
  ...
  settings: '<circle cx="12" cy="12" r="3.1"/><path d="M19.2 14.4a1.5 1.5 0 0 0 .3 1.7l.1.1a1.8 1.8 0 1 1-2.6 2.6l-.1-.1a1.5 1.5 0 0 0-1.7-.3 1.5 1.5 0 0 0-.9 1.4v.2a1.8 1.8 0 1 1-3.6 0V20a1.5 1.5 0 0 0-1-1.4 1.5 1.5 0 0 0-1.7.3l-.1.1a1.8 1.8 0 1 1-2.6-2.6l.1-.1a1.5 1.5 0 0 0 .3-1.7 1.5 1.5 0 0 0-1.4-.9h-.2a1.8 1.8 0 1 1 0-3.6H4a1.5 1.5 0 0 0 1.4-1 1.5 1.5 0 0 0-.3-1.7l-.1-.1a1.8 1.8 0 1 1 2.6-2.6l.1.1a1.5 1.5 0 0 0 1.7.3H9.6a1.5 1.5 0 0 0 .9-1.4V4a1.8 1.8 0 1 1 3.6 0v.2a1.5 1.5 0 0 0 .9 1.4 1.5 1.5 0 0 0 1.7-.3l.1-.1a1.8 1.8 0 1 1 2.6 2.6l-.1.1a1.5 1.5 0 0 0-.3 1.7v.1a1.5 1.5 0 0 0 1.4.9h.2a1.8 1.8 0 1 1 0 3.6H20a1.5 1.5 0 0 0-1.4.9z"/>',
  user: '<circle cx="12" cy="8.2" r="3.9"/><path d="M4.6 20.4c0-3.7 3.3-6.2 7.4-6.2s7.4 2.5 7.4 6.2"/>',
  logout: '<path d="M14.6 4.2H18a2 2 0 0 1 2 2v11.6a2 2 0 0 1-2 2h-3.4"/><path d="M10.2 16.2 14.4 12l-4.2-4.2"/><path d="M14.4 12H4"/>',
...
const icon = (name, size = 22, sw = 1.7) =>
  `<svg class="ico" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="${sw}" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${P[name] ?? (() => { throw new Error('unknown icon: ' + name) })()}</svg>`

const shell = (name, inner) => `<!doctype html>
<html>
<head>
  <meta charset="utf-8">
...
<x-dc>
<helmet>
  <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Bricolage+Grotesque:opsz,wght@12..96,600;12..96,700&family=Schibsted+Grotesk:wght@400;500;600;700&display=swap">
  <style>
${css}  </style>
</helmet>
${inner}
...
let n = 0
for (const file of readdirSync(parts).sort()) {
  if (!file.endsWith('.html')) continue
  const name = file.replace(/\.html$/, '')
  const raw = readFileSync(join(parts, file), 'utf8')
  // {{ico:name}} and {{ico:name,28}} expand to inline SVG
  const inner = raw.replace(/\{\{ico:([a-z]+)(?:,(\d+))?(?:,([\d.]+))?\}\}/g, (_, i, s, w) =>
    icon(i, s ? Number(s) : 22, w ? Number(w) : 1.7))
  writeFileSync(join(here, `${name}.dc.html`), shell(name, inner.trimEnd()))
  n++
}
